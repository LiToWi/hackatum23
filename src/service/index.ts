...
export async function mock_getSavings(
    wallet: WalletContextState
): Promise<Saving[] | null> {
...
export async function blockchain_getSavings(wallet: WalletContextState) {
    ...
    const { program, userAccountPublicKey } = getProgram(wallet);
    const germanAccount = await program.account.user.fetch(
        userAccountPublicKey
    );
    ...
    for (let i = 0; i < userAccount.accounts; i++) {
        ...
        const savings: Saving = {
            accessible: gS.accessible as boolean,
            staking: gS.stalking as boolean,
            accountBalance: gS.accountBalance as number,
            paymentDate: new Date(gS.auszahlDatum as number),
            startDate: new Date(gS.startDate as number),
            goal: gS.goal as number,
            name: gS.name as string,
            id: gS.id as number,
            payments: casted_payments as Payment[],
            paid: 0,
        };
...
export async function blockchain_createSaving(
    wallet: WalletContextState,
    sav: Pick<Saving, 'goal' | 'name' | 'startDate' | 'paymentDate'>
) {
    if (wallet.publicKey == null) {
        return wallet.publicKey;
    }
    ...
    let saving: Saving = {
        ...
        payments: [],
        staking: false,
        paymentDate: sav.paymentDate,
        accountBalance: 0,
        startDate: sav.startDate,
        name: sav.name,
        goal: sav.goal,
        accessible: false,
    };
    ...
    const [savingsAccountPk] = web3.PublicKey.findProgramAddressSync(
        [
            ...
            userMint.toBuffer(),
            new Uint8Array([userAccount.accounts]),
        ],
        programId
    );
    const [savingTokenPk] = web3.PublicKey.findProgramAddressSync(
        [
            Buffer.from('token_savings1'),
            ownerPublicKey.toBuffer(),
            userMint.toBuffer(),
            new Uint8Array([userAccount.accounts]),
        ],
    ...
    const txHash = await program.methods
        .savingInit(
            new BN(sav.goal),
            sav.name,
            new BN(sav.paymentDate.getDate()),
...
export async function blockchain_pay(
    wallet: WalletContextState,
    id: number,
    amount: number
): Promise<boolean> {
