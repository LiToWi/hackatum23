...
export function initCanvas(options: Options) {
    ...
    const feed = (sol: number) => {
        ...
        mood =
            options.payed -
            (0 - options.startTime) *
                (options.goal / (options.endTime - options.startTime)); //TODO insert now for 0
    ...
    coin_konva.on('dragend', function () {
        if (
            coin_konva.x() + coin_konva.getWidth() > piggy_konva.x() &&
            coin_konva.x() + (30 / 450) * piggy_konva.width() <
                piggy_konva.x() + piggy_konva.width() &&
            coin_konva.y() +
                coin_konva.getHeight() -
                (60 / 450) * piggy_konva.height() >
                piggy_konva.y() &&
            coin_konva.y() + 40 < piggy_konva.y() + piggy_konva.height()
        ) {
            //DO STUFF
